...
const ACTIVE_KEY='siaf-cmo-oncologia-active-v1';
...
export function saveActiveCase(state,storage=localStorage){const safe=clone(state);delete safe.clinicalText;if(safe.extraction)for(const trace of Object.values(safe.extraction)){delete trace.evidence;}storage.setItem(ACTIVE_KEY,JSON.stringify(safe));return safe;}
export function getActiveCase(storage=localStorage){try{return JSON.parse(storage.getItem(ACTIVE_KEY)||'null');}catch{return null;}}
export function clearActiveCase(storage=localStorage){storage.removeItem(ACTIVE_KEY);}
